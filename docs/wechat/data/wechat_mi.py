...
def robot(text):
    title = "修改微信运动步数提醒！"
    url = f"http://pushplus.hxtrip.com/send?token=8f8994af17544d77adf86d47700fb60e&title={title}&content={text}&template=html"
    try:
        r = requests.get(url)
    except:
        pass
...
#主函数
def send(user, passwd, step):
    user = str(user)
    password = str(passwd)
    step = str(step)
    if user == '' or password == '' or step == '':
        print ("手机号/密码/步数为空，请重新输入")
        return '手机号/密码/步数为空'
...
if __name__ ==  "__main__":
    step = input('公众号苏生不惑提示你，请输入要修改的微信运动步数：')
    user = input('请输入小米运动手机账号：')
    passwd = input('请输入小米运动密码：')
    msg = send(user, passwd, step)
    robot(msg)